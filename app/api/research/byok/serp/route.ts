...
export async function POST(request: Request) {
  ...
  try {
    const input = await parseByokSerpBody(request);
    if (input.action === "quote") {
      return byokJson(await quoteByokSerp({ ownerId: owner.ownerId, ...input }));
    }
    const decryptionKeys = await loadProviderCredentialDecryptionKeys();
    const result = await executeByokSerp({ ownerId: owner.ownerId, ...input, decryptionKeys });
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await getOwnedByokSerpResult(owner.ownerId, jobId);
    return byokJson(result, { status: result.status === "pending" ? 202 : 200 });
  } catch (error) {
    return byokErrorResponse(error);
  }
